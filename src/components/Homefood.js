...
const Homefood = () => {
  ...
  const Logout = async () => {
    try {
      await axios.delete('https://paimon-recipe-app.herokuapp.com/logout');
      navigate("/");
    } catch (error) {
      console.log(error);
    }
  ...
  const refreshToken = async () => {
    try {
      const response = await axios.get("https://paimon-recipe-app.herokuapp.com/token");
  ...
  const getUsers = async () => {
    const response = await axiosJWT.get("https://paimon-recipe-app.herokuapp.com/users", {
      headers: {
